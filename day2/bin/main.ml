...
let process_line line content =
  ...
  ignore (List.iter (fun group ->
    ...
    Printf.printf "    Max Red: %d; Max Green: %d; Max Blue: %d\n" !r_max !g_max !b_max;
...
let get_valid_games filename = 
  ...
  let rec loop i acc =
    ...
    | line -> loop (i + 1) (acc + ((process_line (i+1) line)))
